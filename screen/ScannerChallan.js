...
class ScannerChallan extends Component {
  constructor(props) {
    super(props);
    this.state = {
      scannedChallan: '',
      torchOn: false, // Add torchOn state
    };
  }
  ...
  onSuccess = (e) => {
    this.setState({
      scannedChallan: e.data,
    });

    // Pass the scanned Challan data to the Data component
    this.props.navigation.navigate('Data', { scannedChallan: e.data });
  };

  render() {
    const { torchOn } = this.state; // Use torchOn for flashlight state
